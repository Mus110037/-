...
import { X, Calendar, DollarSign, AlignLeft, Users, ShieldCheck, Trash2, Zap, Brush, Layers } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" onClick={onClose}>
      <div ref={modalRef} className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-500" onClick={e => e.stopPropagation()}>
        <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2 hover:bg-slate-50 rounded-full text-slate-300"><X className="w-5 h-5" /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8 max-h-[80vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            <div className="space-y-8">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-3 flex items-center gap-2 tracking-widest">企划名称</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-widest">艺术分类</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-widest">金额 (CNY)</label>
            ...
            <div className="space-y-8">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-widest">接稿渠道</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-widest">交付日期</label>
                <input required type="date" className="w-full px-5 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white outline-none font-bold text-slate-900" value={formData.deadline} onChange={e => setFormData({ ...formData, deadline: e.target.value })} />
              </div>

              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-widest">备注信息</label>
                <textarea rows={3} className="w-full px-5 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white outline-none font-medium text-slate-700 text-[11px]" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} placeholder="输入额外要求..." />
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-6">
            {initialOrder && onDelete && (
              <button type="button" onClick={() => { if(confirm('确定删除吗？')) { onDelete(initialOrder.id); onClose(); } }} className="px-6 py-4 rounded-xl bg-slate-100 text-slate-400 hover:text-red-600 transition-all font-bold"><Trash2 className="w-5 h-5" /></button>
            )}
