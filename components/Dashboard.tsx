  ...
  const renderSection = (type: SectionType) => {
    switch (type) {
      case 'upcoming':
        return (
          <div key="upcoming" 
            className={`bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden transition-all ${isEditingLayout ? 'scale-[0.98] border-dashed border-violet-200 ring-4 ring-violet-50' : ''}`}
      ...
      case 'stats':
        return (
          <div key="stats" 
            className={`grid grid-cols-1 md:grid-cols-2 gap-4 transition-all ${isEditingLayout ? 'scale-[0.98]' : ''}`}
            ...
            draggable={isEditingLayout}
          >
            <div className="bg-gradient-to-br from-violet-600 to-indigo-700 p-6 rounded-[2rem] text-white shadow-xl shadow-violet-100 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                ...
                <span className="text-[8px] font-bold uppercase tracking-widest bg-white/10 px-2 py-0.5 rounded-full">预计实收</span>
              </div>
              <div className="relative z-10">
                <p className="text-2xl md:text-3xl font-black">¥{monthlyProjected.toLocaleString()}</p>
                <p className="text-violet-100 text-[9px] mt-1 opacity-80 font-medium">已结入账: ¥{monthlyCompleted.toLocaleString()}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 relative">
              <div className="bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm flex flex-col justify-center items-center text-center">
                 <div className="text-xl md:text-2xl font-black text-slate-900">{orders.filter(o => getProgress(o) < 100).length}</div>
                 <p className="text-[9px] text-slate-400 font-bold uppercase">进行中</p>
              </div>
              <div className="bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm flex flex-col justify-center items-center text-center">
                 <div className="text-xl md:text-2xl font-black text-emerald-500">{orders.filter(o => getProgress(o) === 100).length}</div>
          ...
          >
            <div className="p-4 md:p-6 border-b border-slate-100 flex items-center justify-between bg-white">
              ...
              <button 
                ...
                className="text-violet-600 text-[10px] font-black uppercase tracking-wider bg-violet-50 px-3 py-1.5 rounded-lg"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {orders.map(order => (
                    <button
                      ...
                      className={`p-2.5 rounded-xl border transition-all text-left flex flex-col ${priorityOrderIds.includes(order.id) ? 'bg-violet-50 border-violet-200 ring-2 ring-violet-500/5' : 'bg-white border-slate-100'}`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full bg-violet-500" style={{ width: `${getProgress(order)}%` }}></div>
                          </div>
                          <span className="text-[9px] font-black text-violet-600 w-7">{getProgress(order)}%</span>
                        </div>
                      </div>
                    </div>
                  ))}
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex justify-end">
        <button 
          ...
          className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all shadow-sm ${isEditingLayout ? 'bg-violet-600 text-white ring-4 ring-violet-100' : 'bg-white text-slate-500 border border-slate-100 hover:bg-slate-50'}`}
        >
          <LayoutGrid className="w-3.5 h-3.5" />
          {isEditingLayout ? '保存布局' : '编辑工作台'}
        </button>
      </div>
