...
import { Zap, ArrowUpDown, FileSpreadsheet, MoreHorizontal, Smartphone, Download } from 'lucide-react';
    ...
    <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden animate-in fade-in duration-500">
      <div className="p-6 md:p-8 border-b border-slate-100 flex items-center justify-between bg-white">
        <div className="space-y-1.5">
          <h2 className="text-lg font-bold text-slate-900 tracking-tight uppercase">企划清单</h2>
          <button 
            onClick={() => setSortKey(sortKey === 'deadline' ? 'createdAt' : 'deadline')}
            className="flex items-center gap-1.5 text-[9px] font-bold text-slate-500 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-100 transition-all"
          >
        ...
        {sortedOrders.map((order) => {
          ...
          return (
            <div key={order.id} onClick={() => onEditOrder(order)} className="flex items-center gap-4 p-5 hover:bg-slate-50 transition-all cursor-pointer group">
              <div className="flex flex-col items-center justify-center w-10 h-10 bg-slate-50 border border-slate-200 rounded-xl shrink-0 group-hover:border-slate-400 transition-all">
                <span className="text-[10px] font-bold text-slate-900 leading-none">{order.deadline.slice(8,10)}</span>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden shrink-0 border border-slate-50">
                    <div className="h-full transition-all duration-700" style={{ width: `${stage.progress}%`, backgroundColor: stage.color }} />
                  </div>
                  <span className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">{order.artType} · {order.personCount}</span>
                </div>
              </div>

              <div className="text-right shrink-0">
                <div className="text-[13px] font-bold text-slate-900 leading-none">¥{order.totalPrice}</div>
                <div className="text-[7px] font-bold text-slate-400 mt-1 uppercase tracking-widest">{order.source}</div>
              </div>

              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                <button 
                  onClick={(e) => handleExportSingleICS(e, order)}
                  className="p-2.5 text-slate-400 hover:text-slate-900 bg-white border border-slate-200 rounded-xl shadow-sm"
                  title="同步至日历"
                >
