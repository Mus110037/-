...
const OrderList: React.FC<OrderListProps> = ({ orders, onEditOrder, onDeleteOrder, onUpdateOrder, settings }) => {
  ...
  const handleBulkDelete = () => {
    ...
    if (confirm(`确定要移除选中的 ${selectedIds.length} 个企划吗？`)) {
  ...
  const handleDelete = (orderId: string, e: React.MouseEvent) => {
    ...
    // 使用 requestAnimationFrame 确保在当前事件循环之后触发确认，防止一些移动端浏览器的 UI 冲突
    requestAnimationFrame(() => {
      if (window.confirm('确定要永久删除此企划吗？此操作无法撤销。')) {
    ...
    <div className="space-y-10 pb-32 animate-in fade-in duration-500">
      <div className="flex justify-between items-center px-4">
        <div className="flex items-center gap-6">
          ...
          >
            {isSelectMode ? <><X className="w-4 h-4" /> 取消</> : <><CheckSquare className="w-4 h-4" /> 批量模式</>}
          </button>
          ...
          {isSelectMode && selectedIds.length > 0 && (
            ...
            >
              <Trash2 className="w-4 h-4" /> 确认删除 ({selectedIds.length})
            </button>
          )}
          ...
          {!isSelectMode && (
            <button 
              ...
              className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#7A8B7C] hover:text-[#4B5E4F]"
            >
              <ArrowUpDown className="w-4 h-4" />
              {sortMethod === 'deadline' ? '按交付时间' : '按录入时间'}
            </button>
          )}
        </div>
      </div>
            ...
            {group.orders.map((order) => {
              ...
              return (
                <div 
                  ...
                  className={`bg-[#FDFBF7] flex items-stretch rounded-2xl border transition-all cursor-pointer group shadow-sm ${isSelected ? 'border-[#4B5E4F] bg-emerald-50/20 ring-2 ring-[#4B5E4F]/10' : 'border-[#D6D2C4] hover:border-[#4B5E4F]'}`}
                >
                  <div className={`w-1.5 rounded-l-2xl shrink-0 ${isHigh ? 'bg-[#D4A373]' : 'bg-[#D6D2C4]'}`} />
                  
                  <div className="flex-1 flex items-center gap-2 md:gap-5 p-3 md:p-5 overflow-hidden">
                    {isSelectMode && (
                      <div className="shrink-0 mr-1">{isSelected ? <CheckSquare className="w-6 h-6 text-[#4B5E4F]" /> : <Square className="w-6 h-6 text-[#D6D2C4]" />}</div>
                    )}
                    
                    <div className="flex flex-col items-center justify-center w-9 h-9 md:w-11 md:h-11 bg-[#3D4C41] rounded-xl shrink-0">
                      ...
                      <span className="text-[7px] md:text-[9px] font-bold text-[#A3B18A] uppercase mt-0.5">{format(new Date(order.deadline.replace(/-/g, '/')), 'MMM', { locale: zhCN })}</span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0 px-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="px-1 py-0.5 bg-[#EDE9DF] text-[#4B5E4F] border border-[#D6D2C4] rounded text-[8px] font-black uppercase shrink-0">{getShortSource(order.source)}</span>
                    ...
                    <div className="ml-auto flex items-center gap-1.5 md:gap-3 shrink-0">
                      <div className="flex items-center gap-1">
                        <button 
                          ...
                          className="p-2 md:p-2.5 rounded-lg border bg-white text-[#4B5E4F] border-[#D6D2C4] hover:bg-slate-50 shadow-sm flex items-center justify-center shrink-0 active:scale-95 transition-transform"
                          title="同步至日历"
                        >
                        ...
                        <button 
                          ...
                          className="p-2 md:p-2.5 rounded-lg border bg-slate-50 text-slate-400 border-[#D6D2C4] hover:text-red-600 hover:bg-red-50 hover:border-red-200 active:scale-95 transition-all shadow-sm flex items-center justify-center shrink-0"
                          title="删除企划"
                        >
